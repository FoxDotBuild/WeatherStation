...
var _a = require('./render'), MdRenderer = _a.MdRenderer, renderEmoji = _a.renderEmoji, formatMsgs = _a.formatMsgs, wrapPage = _a.wrapPage, renderThread = _a.renderThread;
...
module.exports = function makeServe(sbot, config) {
    ...
    return function serve(req, res) {
        ...
        if (req.url.startsWith("/user-feed/"))
            return serveUserFeed(req, res, m[4]);
    ...
    function serveFeed(req, res, feedId) {
        ...
        pull(sbot.createUserStream({ id: feedId, reverse: true }), pull.collect(function (err, logs) {
            if (err)
                return respond(res, 500, err.stack || err);
            res.writeHead(200, {
                "Content-Type": ctype("html")
            });
            pull(pull.values(logs), paramap(addAuthorAbout, 8), paramap(addFollowAbout, 8), paramap(addVoteMessage, 8), pull(renderThread(opts), wrapPage(feedId)), toPull(res, function (err) {
                if (err)
                    debug("ERROR %O", err);
            }));
        }));
    ...
    function serveUserFeed(req, res, url) {
        ...
        pull(sbot.createUserStream({ id: feedId }), pull.filter(function (msg) {
            return (!msg.value ||
    ...
    function serveFeeds(req, res, following, channelSubscriptions, feedId) {
        ...
        pull(sbot.createLogStream({ reverse: true, limit: 2500 }), pull.filter(function (msg) {
            return (!msg.value ||
                (msg.value.author in following ||
                    msg.value.content.channel in channelSubscriptions));
        }), pull.take(100), pull.collect(function (err, logs) {
            if (err)
                return respond(res, 500, err.stack || err);
            res.writeHead(200, {
                "Content-Type": ctype("html")
    ...
    function serveChannel(req, res, url) {
        ...
        pull(sbot.query.read({
            ...
            pull(pull.values(logs), paramap(addAuthorAbout, 8), paramap(addVoteMessage, 8), pull(renderThread(opts), wrapPage(channelId)), toPull(res, function (err) {
                if (err)
                    debug("ERROR %O", err);
            }));
        }));
    ...
    function serveId(req, res, id, ext, query) {
        ...
        if (!base) {
            if (ext === "js")
                base = baseToken = "__BASE_" + Math.random() + "_";
            else
                base = "/";
        }
        var opts = {
        ...
        if (format === null)
            return respond(res, 415, "Invalid format");
        pull(sbot.links({ dest: id, values: true, rel: "root" }), includeRoot && prepend(getMsg, id), pull.unique("key"), pull.collect(function (err, links) {
            if (err)
                return respond(res, 500, err.stack || err);
            ...
            pull(pull.values(sort(links)), paramap(addAuthorAbout, 8), format, toPull(res, function (err) {
                if (err)
                    debug("ERROR %O", err);
            }));
        }));
    ...
    sbot.blobs.has(id, function (err, has) {
        if (err) {
            if (/^invalid/.test(err.message))
                ...
                return respond(res, 500, err.message || err);
        }
        if (!has)
        ...
        pull(sbot.blobs.get(id), toPull(res, function (err) {
            if (err)
                debug("ERROR %O", err);
        }));
    });
...
function serveFile(req, res, file) {
    fs.stat(file, function (err, stat) {
        ...
        if (err)
            return respond(res, 500, err.stack || err);
        if (!stat.isFile())
            return respond(res, 403, "May only load files");
        if (ifModified(req, stat.mtime))
            return respond(res, 304, "Not modified");
        res.writeHead(200, {
